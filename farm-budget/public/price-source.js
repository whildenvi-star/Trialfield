      ...
      (row.variants || []).forEach(function (v) {
        (v.budgetCropNames || []).forEach(function (name) {
          byCrop[String(name).trim().toLowerCase()] = {
            ...
            poolFutures: row.poolFuturesWapCents != null ? row.poolFuturesWapCents / 100 : null,
            poolBlend: row.poolFuturesBlendCents != null ? row.poolFuturesBlendCents / 100 : null,
            poolPctSold: row.pctSold != null ? row.pctSold : null,
            poolSoldBu: row.soldBu || 0,
            poolProjectedBu: row.projectedBu != null ? row.projectedBu : null,
